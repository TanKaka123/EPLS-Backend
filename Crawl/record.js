      ...
      if (!error && response.statusCode == 200) {
        ...
        $(".statsCard").each((index, el) => {
          ...
          if (index <= 2 || index==17 || index==13 ) {
            ...
            for(let i=2;i<=5;i++)
            {
              const position = $(el).find(`.statsRow:nth-child(${i})`).find(".pos").text();
              const name = $(el).find(`.statsRow:nth-child(${i})`).find(".statName").text();
              const clbName = $(el)
        ...
        fs.writeFileSync(`./Data/Record/${title.replace(' ','')}.json`, JSON.stringify(data));
          }
        });
      } else {
        console.log(error);
